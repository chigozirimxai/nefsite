...
if (isset($_POST['reset'])) {
	...
	if ($email=="" || $password1=="" || $password2==""){
		...
		$php= "SELECT * FROM `clanlogin` WHERE(`email`='$email')";
		$query= mysqli_query($connect,$php);
		$count= mysqli_num_rows($query);

		if($count>0) {
			$php= "UPDATE `shoplogin` SET `password`='$password1' WHERE(`email`='$email')";
			$query= mysqli_query($connect,$php);
			if ($query) {
...
<head>
	<meta charset="utf-8">
	<title>CLAN LEADER'S RESET PASSWORD</title>
		...
		<div class="row">
			...
			<div class="col-md-6">
				...
				<form class="resetpassword_form" method="post">
						<div class="title">
			<h1 class="name"> CLAN RESET PASSWORD</h1>
		</div>
					...
					<p><a href="shopadminlogin.php">login</a></p>
				</form><br>
			</div>
			<div class="col-md-3"></div>
